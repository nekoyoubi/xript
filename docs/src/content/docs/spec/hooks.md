---
title: Hooks
description: "Hooks are the host-to-mod lifecycle-event direction: the host fires them, scripts handle them."
---

Hooks are [bindings](/spec/bindings) run backwards: the host fires them, scripts handle them. Bindings let scripts call the host; hooks let the host call scripts. Put the two together and you have a bidirectional channel between the application and its mods.

:::caution[Hooks are an event-typed slot]
A lifecycle hook is a [slot](/spec/manifest/#slots) whose `accepts` type is the event-handler kind (`application/x-xript-hook`). A mod handles a hook by filling that slot with `{ "handler": "onPlayerDamage" }`, and firing the hook means calling the slot's fills. The standalone `hooks` manifest field documented below remains allowed for back-compat, but new hosts should declare event-typed slots. Host-side hook firing is unchanged either way.
:::
...
:::note[Don't confuse this with the `events` catalog]
Three surfaces touch "events," and they answer different questions. **Event-typed slots** (this page) are extension points the host declares and a mod fills with a handler the host fires. The top-level [`events` catalog](/spec/manifest/#events) declares what the *host broadcasts* and the payload each event carries; it wires up no listener. A fragment's [`handlers`](/spec/fragments/#event-handlers) are DOM responses on a mounted fragment fill (`events` there is a deprecated alias for that handler array, distinct from the catalog's `events`). In one line: bindings are *what you call*, slots and fragment handlers are *what handles*, and the `events` catalog is *what the host emits*.
:::

## Declaration

A host declares an event as a [slot](/spec/manifest/#slots) whose `accepts` is the event-handler type:
...
A mod fills that slot by naming a handler export in its [mod manifest](/spec/mod-manifest/)'s `fills` surface, keyed by the slot id:
