...
- **`risk`** (`low`, `medium`, `high`, default `low`): An advisory level that helps users make informed decisions. Does not affect runtime behavior.
- **`reserved`** (boolean, default `false`): Marks a capability that is intentionally declared without yet gating any binding, slot, or hook; inherited for parity, or held for a future surface. Tooling treats a `reserved` capability as aspirational rather than vestigial: the unreferenced-capability lint is suppressed, and it is excluded from coverage denominators rather than counted as dead.
...
The manifest does not support listing multiple capabilities per function. This is intentional: the simplicity of "one function, one gate" makes the model easy to reason about for both authors and modders.
...
These are host-side runtime options, not manifest-declared fields. The manifest schema does not change for them.

- **Cancellation**: a host-held cancellation token on the runtime options. Flipping it interrupts the in-flight execution cooperatively at the next interrupt check, surfacing a cancellation error distinct from a timeout. Cancellation is sticky and idempotent; a fresh execution on a cancelled token errors immediately. Dropping the runtime does not auto-cancel.
...
- **Debug**: an optional debug option attaches a DAP-shaped debug session before execution (see [Debug Protocol](./debug-protocol.md)). It is default-off with zero overhead when absent, mirroring the audit channel. xript imposes no capability for it; attaching a debugger is host policy, not an xript capability.

## Grant Shapes (host-side)

Granting a capability flows the same way regardless of these shapes: the host passes capability names into the runtime options. The runtimes never see a grant prompt, a grant decision, or any of the shapes below — **grant policy and prompt UX are host-side, mechanism-not-policy.**
...
- **`DiscoveryResult`** (`discovery-result.schema.json`) — what an addon-discovery pass yields: a `mods` array (each with `name`, `version`, `location`, `enabled`, `capabilities`, and `provides`) plus `scannedAt`. The `provides` entries are logical roles, sharing the vocabulary of mod-manifest role contributions.

A prompt payload merely **describes** a request. Granting still happens host-side through the runtime options; default-deny is preserved.
