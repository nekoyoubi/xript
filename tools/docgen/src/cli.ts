...
const grantShapes = args.includes("--grant-shapes");
const positional = args.filter((a) => a !== "--grant-shapes");
...
if (outputIndex !== -1) {
	outputDir = positional[outputIndex + 1];
	if (!outputDir) {
...
try {
	const result = await generateDocsFromFile(manifestPath, { grantShapes });
