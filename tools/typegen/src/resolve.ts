...
function extendsPaths(manifest: Json): string[] {
	...
	throw new ManifestResolutionError("extends must be a string or an array of strings");
}
...
function mergeMaps(base: unknown, child: unknown, mapKey: string): Json {
	...
	for (const [key, value] of Object.entries(baseMap)) merged[key] = value;
	for (const [key, value] of Object.entries(childMap)) {
		if (key in baseMap) {
			const singular = mapKey === "capabilities" ? "capability" : mapKey.replace(/s$/, "");
			throw new ManifestResolutionError(`${singular} id "${key}" conflicts with extended base`);
		}
		merged[key] = value;
	}
	return merged;
...
function mergeSlots(base: unknown, child: unknown): unknown[] {
	...
	for (const slot of childSlots) {
		if (isObject(slot) && typeof slot.id === "string") {
			if (seen.has(slot.id)) {
				throw new ManifestResolutionError(`slot id "${slot.id}" conflicts with extended base`);
			}
			seen.add(slot.id);
		}
		out.push(slot);
...
function mergeManifests(base: Json, child: Json): Json {
	...
	for (const key of SCALAR_KEYS) if (key in child) result[key] = child[key];
	for (const key of MAP_KEYS) {
		if (key in base || key in child) result[key] = mergeMaps(base[key], child[key], key);
	}
	if ("slots" in base || "slots" in child) result.slots = mergeSlots(base.slots, child.slots);
...
async function readManifestFile(filePath: string): Promise<Json> {
	...
	try {
		...
		throw new ManifestResolutionError(`could not read extended manifest: ${filePath}`);
	}
	let parsed: unknown;
	try {
		parsed = JSON.parse(raw);
	} catch {
		throw new ManifestResolutionError(`invalid JSON in extended manifest: ${filePath}`);
	}
	if (!isObject(parsed)) throw new ManifestResolutionError(`extended manifest is not an object: ${filePath}`);
	return parsed;
}

async function resolveWithChain(manifest: Json, baseDir: string, chain: string[]): Promise<Json> {
	...
	for (const relPath of paths) {
		...
		if (chain.includes(absPath)) {
			throw new ManifestResolutionError(`cyclic extends detected: ${[...chain, absPath].join(" -> ")}`);
		}
