...
describe("cancellation", () => {
	it("immediately errors on execute after cancel", () => {
		...
		const runtime = createRuntime(minimalManifest, { hostBindings: {}, cancellation: token });
...
describe("host-invokable exports", () => {
	...
	it("invokes a registered export and honors the return value", () => {
		const runtime = createRuntime(appManifest, { hostBindings: {} });
		loadExportMod(
			runtime,
	...
	it("gates an export behind a declared capability", () => {
		const runtime = createRuntime(appManifest, { hostBindings: {}, capabilities: [] });
		loadExportMod(
			runtime,
