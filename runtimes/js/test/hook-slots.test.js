...
describe("event-typed-slot hooks — dispatch (#112)", () => {
	it("injects a hooks registration verb for an event-typed slot", () => {
		const runtime = xript.createRuntime(slotHookManifest, { hostBindings: {} });
		runtime.execute(`
			globalThis.__seen = [];
			hooks.playerDamage(function(amount, source) {
				globalThis.__seen.push([amount, source]);
			});
		`);
	...
	it("admits a gated slot-hook when a subsuming grant is held", () => {
		const runtime = xript.createRuntime(slotHookManifest, {
			hostBindings: {},
	...
	it("an explicit hooks-block entry wins over a same-named slot", () => {
		...
		runtime.execute(`
			globalThis.__phase = [];
			hooks.playerDamage.pre(function() { globalThis.__phase.push("pre"); });
		`);
		runtime.fireHook("playerDamage", { phase: "pre" });
		assert.deepEqual(runtime.execute("globalThis.__phase").value, ["pre"]);
		runtime.dispose();
	});
});
