...
before(async () => {
	...
	xriptAsync = await initXriptAsync();
});
...
describe("provider roles — resolution", () => {
	...
	it("rolePreferences override the winner", () => {
		const rt = xript.createRuntime(appManifest, {
			hostBindings: {},
...
describe("record schemas — manifest tolerance", () => {
	it("tolerates field default and inline enum metadata without error", () => {
		...
		const rt = xript.createRuntime(manifest, { hostBindings: {} });
...
describe("debug protocol — js (async sandbox)", () => {
	it("createSandboxSync (initXript) rejects debug option", () => {
		assert.throws(
			() => xript.createRuntime(appManifest, { hostBindings: {}, debug: { onStopped() {} } }),
			DebugUnsupportedError,
		);
	});

	it("debugSession is null when debug is not set", async () => {
		const rt = await xriptAsync.createRuntime(appManifest, { hostBindings: {} });
		assert.equal(rt.debugSession(), null);
		rt.dispose();
	});

	it("attaches a debug session with instrumented fidelity", async () => {
		const rt = await xriptAsync.createRuntime(appManifest, { hostBindings: {}, debug: { onStopped() {} } });
	...
	it("pauses at a breakpoint and inspects locals", async () => {
		const stopped = [];
		const rt = await xriptAsync.createRuntime(appManifest, {
			hostBindings: {},
	...
	it("a pause request stops at the next statement boundary", async () => {
		const stopped = [];
		const rt = await xriptAsync.createRuntime(appManifest, {
			hostBindings: {},
			debug: { onStopped: (e) => stopped.push(e) },
		...
		await new Promise((r) => setTimeout(r, 80));
		assert.equal(stopped.length, 1);
		assert.equal(stopped[0].reason, "pause");
		assert.equal(sess.stackTrace()[0].line, 1);
		sess.continue();
		await p;
		rt.dispose();
	});

	it("evaluate reports unsupported uniformly", async () => {
		const rt = await xriptAsync.createRuntime(appManifest, {
			hostBindings: {},
			debug: { onStopped() {} },
		});
		const sess = rt.debugSession();
		const v = sess.evaluate("1 + 1");
